...
if [ "$#" -eq 2 ]; then
    ...
    gst-launch-1.0 filesrc location="$input_path" ! decodebin name=decoder ! queue ! videoconvert ! videoscale ! x264enc ! qtmux name=muxer ! filesink location="$output_path" decoder. ! audioconvert ! audioresample ! queue ! avenc_aac ! muxer.
